...
class BoardController extends Controller
{
    ...
    public function deleteAction($id)
    {
        ...
        $flashMessageData = array(
            'title' => 'Deletion successful', 'message' => 'Kanban board "' . $board->getName() . '" was deleted!',
        );
        $this->get('session')
                ->setFlash('success', $flashMessageData);
    ...
    public function newAction(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($form->isValid()) {
                ...
                $flashMessageData = array(
                    'title' => 'Creation successful', 'message' => 'Kanban board "' . $board->getName() . '" was created!',
                );
                $this->get('session')
                        ->setFlash('success', $flashMessageData);
    ...
    public function editAction($id, Request $request)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($form->isValid()) {
                ...
                $query = $entityManager->createQuery('SELECT user
					...
					WHERE
						board.id = :boardId')
                                       ->setParameters(array('boardId' => $board->getId()));
                ...
                // Build the success message for the user
                $flashMessageData = array(
                    'title' => 'Editing successful', 'message' => 'Kanban board "' . $board->getName() . '" was edited!',
                );
                $this->get('session')
                        ->setFlash('success', $flashMessageData);
        ...
        $templateData = array(
            'form' => $form->createView(), 'board' => $board
        );
    ...
    public function listAction()
    {
        ...
        $entityManager = $this->getDoctrine()
                              ->getEntityManager();
        $boards = $entityManager->getRepository('DigitalKanbanBaseBundle:Board')
                                ->findAll();
    ...
    public function updateAction()
    {
        ...
        $issueIds = explode(',', $request->request
                                         ->get('issues'));
        ...
        if ($request->isXmlHttpRequest() === FALSE || ($boardColumnController->isUserAllowedToEditThisColumn($user, $columnId, $entityManager) === FALSE && $columnId > 0)) {
            return new Response(NULL, 403);
        }

        if ($columnId == 0) {
            //archiving
            $issuetoarchive = $entityManager->getRepository('DigitalKanbanBaseBundle:Issue')
                                            ->find($issueIds);
            ...
            $unsortedIssues = $entityManager->getRepository('DigitalKanbanBaseBundle:Issue')
                                            ->findById($issueIds);
